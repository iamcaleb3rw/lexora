// utils/resumeGenerator.ts
import { StructuredResume } from "@/types/resume";

export const generateHTML = (resume: StructuredResume) => {
  const {
    fullName,
    emailAddress,
    phoneNumber,
  ...
  // Function to ensure unique IDs for AI targeting
  const generateSegmentId = (type: string, index: number) =>
    `seg_${type}_${index}`;
          ...
          (exp, index) =>
            ` <div style="margin-bottom: 1rem; page-break-inside: avoid;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.25rem;">
                  <div>
                    <strong style="font-size: 1.1rem;">${exp.title}</strong> ${exp.company ? ` - ${exp.company}` : ""}
                  </div>
                  <div style="color: #666; font-size: 0.9rem; text-align: right;">
                    ${exp.startDate || ""} ${exp.endDate ? ` - ${exp.endDate}` : exp.startDate ? " - Present" : ""}
                  </div>
                </div>
                ${exp.description ? `<div data-segment-id="${generateSegmentId("exp", index)}" style="margin-top: 0.25rem; line-height: 1.4;">${exp.description.replace(/\n/g, "<br>")}</div>` : ""}
              </div> `
        )
        .join("")
        ...
        .map(
          (edu, index) =>
            ` <div style="margin-bottom: 1rem; page-break-inside: avoid;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.25rem;">
                  <div>
                    <strong style="font-size: 1.1rem;">${edu.degree}</strong> ${edu.institution ? ` - ${edu.institution}` : ""}
                  </div>
                  <div style="color: #666; font-size: 0.9rem; text-align: right;">
                    ${edu.startDate || ""} ${edu.endDate ? ` - ${edu.endDate}` : edu.startDate ? " - Present" : ""}
                  </div>
                </div>
                ${edu.description ? `<div data-segment-id="${generateSegmentId("edu", index)}" style="margin-top: 0.25rem; line-height: 1.4;">${edu.description.replace(/\n/g, "<br>")}</div>` : ""}
              </div> `
        )
        .join("")
  ...
  const skillsHTML = skills.length
    ? `<div data-segment-id="${generateSegmentId("skills", 0)}" style="line-height: 1.8;">${skills.map((skill) => `${skill}`).join(" • ")}</div>`
    ...
    <head>
      ...
      <style>
        body {
          ...
          margin: 0 auto;
          padding: 20px;
          overflow: hidden; /* Prevent editor scrollbars from showing up in print */
        }
        @media print {
          body {
            padding: 0;
